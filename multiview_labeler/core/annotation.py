...
class AnnotationManager(QtCore.QObject):
    ...
    def _normalize_frame(self, frame: FrameAnnotations) -> FrameAnnotations:
        camera_ids = self.dataset.camera_ids()
        for cid in camera_ids:
            keypoints = frame.by_camera.setdefault(cid, ensure_keypoints(len(KEYPOINTS)))
            if len(keypoints) < len(KEYPOINTS):
                keypoints.extend(ensure_keypoints(len(KEYPOINTS) - len(keypoints)))
            elif len(keypoints) > len(KEYPOINTS):
                del keypoints[len(KEYPOINTS):]
        if len(frame.points3d) < len(KEYPOINTS):
            frame.points3d.extend([None] * (len(KEYPOINTS) - len(frame.points3d)))
        elif len(frame.points3d) > len(KEYPOINTS):
            del frame.points3d[len(KEYPOINTS):]
        return frame
    ...
    def frame(self, frame_idx: int, instance_idx: int = 0) -> FrameAnnotations:
        ...
        return self._normalize_frame(self.frames[frame_idx][instance_idx])
    ...
    def restore(self, frame_idx: int, instance_idx: int, snapshot: str) -> None:
        ...
        if frame_idx not in self.frames:
            self.frames[frame_idx] = {}
        self.frames[frame_idx][instance_idx] = self._normalize_frame(
            FrameAnnotations(
