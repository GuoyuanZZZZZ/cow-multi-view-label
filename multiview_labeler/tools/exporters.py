...
class Exporter:
    ...
    def export_2d_csv(path: Path, annotations: AnnotationManager) -> None:
        with path.open("w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["frame", "camera", "keypoint", "x", "y", "visibility"])
            for frame_idx, frame in sorted(annotations.frames.items()):
                for cid, kps in frame.by_camera.items():
                    for idx, kp in enumerate(kps):
                        writer.writerow([frame_idx, cid, KEYPOINTS[idx], kp.x, kp.y, kp.visibility])
        ...
        with path.open("w", newline="", encoding="utf-8") as f:
            ...
            for frame_idx, frame in sorted(annotations.frames.items()):
                for idx, point in enumerate(frame.points3d):
                    writer.writerow([frame_idx, KEYPOINTS[idx], *(point or [None, None, None])])
